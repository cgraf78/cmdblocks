...
#
# Linking rather than copying keeps each self-contained command and its
# documentation on one reviewed revision without creating a second program
# copy that can drift from this checkout.
  ...
  for ((index = count - 1; index >= 0; index--)); do
    ...
    if [[ "${prior_states[$index]}" == symlink ]]; then
      if ! ln -sfn -- "${prior_targets[$index]}" "$target"; then
        printf 'cmdblocks: rollback failed to restore symlink: %s\n' "$target" >&2
      fi
    elif ! rm -f "$target"; then
      printf 'cmdblocks: rollback failed to remove new link: %s\n' "$target" >&2
    fi
  done
...
printf 'installed cmdblocks commands to %s\n' "$BIN_DIR"
printf 'installed cmdblocks manpages to %s\n' "$MAN_DIR"
